      ...
      <section className="carbon relative min-h-screen py-8">
        ...
        <div className="container relative z-10">
          <nav className="flex items-center justify-between py-5">
            <Link
              href="/#team"
              className="inline-flex items-center gap-3 text-xs font-bold uppercase tracking-[0.18em] text-white/68 transition hover:text-white"
            >
              <ArrowLeft size={18} />
              Back to Team
            </Link>
          ...
          <div className="grid gap-8 py-12 lg:grid-cols-[1fr_0.7fr] lg:items-end">
            ...
            <div className="border-l-2 border-[#ff5400] bg-black/35 p-6">
              ...
              <p className="mt-4 text-sm leading-7 text-white/60">
                Different branches bring different ways of thinking to the same
                car. Meet the students turning that mix of ideas into dependable
                work on track.
              </p>
            </div>
          </div>
          ...
          <section aria-labelledby="member-heading">
            ...
            <div className="grid gap-14">
              {cohorts.map(({ batch, graduationYear }) => {
                const members = subteam.members.filter(
                ...
                return (
                  <section key={batch} aria-labelledby={`${batch}-heading`}>
                    <div className="mb-6 flex items-end gap-4 border-b border-white/10 pb-4">
                      ...
                      <p className="pb-1 text-sm text-white/42">
                    ...
                    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                      {members.map((member) => (
                        <article
                          key={`${member.name}-${member.graduationYear}`}
                          className="group overflow-hidden rounded-[10px] border border-white/10 bg-[#111216] transition hover:-translate-y-1 hover:border-[#ff5400]/55 hover:shadow-[0_24px_70px_rgba(0,0,0,0.35)]"
                        >
                          ...
                          <div className="p-5">
                            ...
                            <div className="mt-3 min-h-[5.5rem]">
                              {member.position ? (
                                <>
                                  <p className="font-telemetry text-[9px] font-bold uppercase tracking-[0.18em] text-white/38">
                                    Position of Responsibility
                                  </p>
                                  <p className="mt-1 text-sm font-semibold leading-6 text-[#ff5400]">
                                    {member.position}
                                  </p>
                                </>
                              ) : null}
